...
class IN_Attention(nn.Module):
    ...
    def forward(self, x, attn_guide = None):
        B, N, C = x.shape
        ...
        if attn_guide != None:
            tradeoff = 1.0
            attn = tradeoff * attn + (1-tradeoff) * attn_guide[-1][0].detach()
...
class OUT_Attention(nn.Module):
    ...
    def forward(self, x, r, attn_guide = None):
        ...
        if attn_guide != None:
            tradeoff = r  # 0.7
            attn = tradeoff * attn + (1-tradeoff) * attn_guide[-1][1].detach()
...
class Block(nn.Module):
    ...
    def forward(self, pixel_embed, patch_embed, r, attn_guide = None):
        # inner
        temp1, inner_attn = self.attn_in(self.norm_in(pixel_embed))
        pixel_embed = pixel_embed + self.drop_path(temp1)
        pixel_embed = pixel_embed + self.drop_path(self.mlp_in(self.norm_mlp_in(pixel_embed)))
        # outer
        B, N, C = patch_embed.size()
        patch_embed[:, 1:] = patch_embed[:, 1:] + self.proj(self.norm1_proj(pixel_embed).reshape(B, N - 1, -1))
        
        temp2, outer_attn = self.attn_out(self.norm_out(patch_embed), r, attn_guide)
        patch_embed = patch_embed + self.drop_path(temp2)
        patch_embed = patch_embed + self.drop_path(self.mlp(self.norm_mlp(patch_embed)))
        return pixel_embed, patch_embed, (inner_attn, outer_attn)
...
class TNT(nn.Module):
    ...
    def forward_features(self, x, r, attn_guide = None):
        B = x.shape[0]
        ...
        for blk in self.blocks:
            pixel_embed, patch_embed, attn = blk(pixel_embed, patch_embed, r, attn_guide)
            layer_wise_tokens.append(patch_embed)
            attns.append(attn)
            embeds.append((pixel_embed, patch_embed))

        x = self.norm(patch_embed)
        patch_embed = [self.norm(x) for x in layer_wise_tokens]
        # return [y[:, 0] for y in patch_embed], layer_wise_tokens
        return x[:,0], attns, [y[:,0] for y in patch_embed]

    def forward(self, x, r = 0.7, attn_guide = None):
        # x_list, layer_wise_tokens = self.forward_features(x)
        # x = [self.head(x) for x in x_list]
        x, attns, layer_wise = self.forward_features(x, r, attn_guide)
        x = self.head(x)
        return x, attns, [self.head(x) for x in layer_wise]
