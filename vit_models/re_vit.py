...
from timm.models.vision_transformer import _cfg, default_cfgs, checkpoint_filter_fn, _init_vit_weights, _load_weights, resize_pos_embed
import random
import numpy as np
...
class ReBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = ReAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
class MyVit(nn.Module):
    ...
    def forward_features(self, x, r, global_attn = None):
        ...
        x = self.pos_drop(x + self.pos_embed)
        cls_outputs = []
        attns = []  # 保存每一个block的attention map
        keep = torch.ones(12,dtype=torch.bool)
        cls_tocken = None
        for idx, blk in enumerate(self.blocks):
            
            x, attn = blk(x, r, global_attn = global_attn)
            attns.append(attn)
            cls_outputs.append(x)
        ...
        # x = self.blocks(x)
        x = self.norm(x)
        cls_outputs = [self.norm(y) for y in cls_outputs]
        pred = [self.pre_logits(x[:, 0]) for x in cls_outputs]
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), attns, pred  # 8,768
        else:
            return x[:, 0], x[:, 1]
    ...
    def forward(self, x, r = 1.0, global_attn = None):
        ...
        Parameters
        ----------
        K : 从前一层继承attention比例      range:0~12
        P : 哪些block继承之前的attention   range:0~12
        '''
        x, attn_lst, layer_wise = self.forward_features(x,r,global_attn = global_attn)
        layer_wise = [self.head(x) for x in layer_wise]
        if self.head_dist is not None:
            x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
            ...
            x = self.head(x), attn_lst, layer_wise
        return x
