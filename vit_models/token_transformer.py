...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape     # bs,56^2,147

        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.in_dim).permute(2, 0, 3, 1, 4)  # bs,56*56,3,1,64 --> 3,bs,1,56*56,64
        q, k, v = qkv[0], qkv[1], qkv[2]  # bs,1,56*56,64

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, self.in_dim)  # bs,56*56,64
        x = self.proj(x)
        x = self.proj_drop(x)
...
class Token_transformer(nn.Module):
    ...
    def forward(self, x):  # bs,56*56,147
