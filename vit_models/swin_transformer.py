...
def restruct_patchs(patchs, seed, prob = 0.3):
    bs, num_patch, dim = patchs.shape
    ids = [n for n in range(num_patch)]
    p = np.random.rand(1)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None, global_attn = None):
        """
        Args:
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, global_attn = None):
        ...
        attn_windows, attn = self.attn(x_windows, mask=self.attn_mask, global_attn = global_attn)  # nW*B, window_size*window_size, C
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x, global_attn = None):
        ...
        for layer_idx, cur_layer in enumerate(self.layers):
            if global_attn!=None:
                stage_attn = global_attn[layer_idx][-1].detach()
                
            else:
                stage_attn = None
            x, attn = cur_layer(x, global_attn = stage_attn)  
        ...
        return x, layer_attns

    def forward(self, x, global_attn = None):
        x, attn = self.forward_features(x, global_attn = global_attn)
